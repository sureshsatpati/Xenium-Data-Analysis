...
Xenium Data Analysis with ArchR

This repository contains code and resources for analyzing Xenium single-cell spatial transcriptomics data using ArchR. Xenium provides highly multiplexed gene expression and spatial localization data, allowing for a deeper understanding of tissue architecture and gene activity at a single-cell resolution. The analysis pipeline integrates spatial and transcriptomic data, enabling the identification of spatially regulated gene expression patterns within tissues.

# Key Features:

Spatial Transcriptomics: Integrates Xenium single-cell spatial data with multi-omic analysis, enabling spatially resolved gene expression analysis.

Gene Expression Mapping: Visualizes gene expression patterns across tissue sections and links these patterns to specific spatial regions.

Cell Type Annotation: Identifies and annotates cell types based on both gene expression and spatial location, revealing tissue organization and heterogeneity.

Spatial Clustering: Uses spatial clustering methods to identify regions of interest with similar gene expression profiles, helping to map tissue organization at a finer scale.

Integration with scRNA-seq and scATAC-seq: Combines data from different assays for a comprehensive view of spatial gene regulation and chromatin accessibility.

Interactive Visualization: Interactive plots for exploring gene expression profiles across spatially distinct regions of tissue sections.

# Requirements:

ArchR (for multi-omic data analysis)

R (and required R libraries)

Xenium spatial data files (for single-cell transcriptomics with spatial information)
...
makeShinyApp(alldata, scConf,gex.assay = "SCT",gene.mapping = TRUE, shiny.dir = "Reg1-3_Xenium_LPS10_MP_together_Shiny", shiny.title = "Reg1-3_Xenium_LPS10_Matched_Pair_Together",shiny.footnotes = "by Satpati")
